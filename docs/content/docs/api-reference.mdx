...
- `studio.addTrack(track: StudioTrack, index?: number)`: Create a new track.
- `studio.moveTrack(trackId: string, newIndex: number)`: Reorder a track.
- `studio.setTrackOrder(trackIds: string[])`: Set absolute track order.
- `studio.removeTrack(trackId: string)`: Remove a track and its clips.
