...
studio.on('track:added', ({ track, index }) => {
  console.log(`Track added at index ${index}:`, track.name);
});
...
Explicitly create a new track. You can optionally specify an `index` to control its layering position.

```ts
const newTrack = studio.addTrack({ name: "Subtitles", type: "Text" }, 0);
```

### moveTrack()

Move an existing track to a new index.

```ts
await studio.moveTrack(trackId, 2);
```

### setTrackOrder()

Reorder all tracks by providing an array of track IDs.

```ts
await studio.setTrackOrder(["t1", "t2", "t3"]);
```

### removeTrack()

Remove a track and all its associated clips.
...
```ts
import { Studio, Image, Text } from "openvideo";
